...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
					...
					<?php next_post_link( '%link', '%title ' . _x( '&rarr;', 'Next post link', TabulaRasa::get_textdomain() ) . '' ); ?>
...
<?php if ( get_the_author_meta( 'description' ) ) : // If a user has filled out their description, show a bio on their entries  ?>
							...
							<h2><?php printf( esc_attr__( 'About %s', TabulaRasa::get_textdomain() ), get_the_author() ); ?></h2>
							<?php the_author_meta( 'description' ); ?>
							<a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>">
								<?php printf( __( 'View all posts by %s &rarr;', TabulaRasa::get_textdomain() ), get_the_author() ); ?>
							</a>
<?php endif; ?>
